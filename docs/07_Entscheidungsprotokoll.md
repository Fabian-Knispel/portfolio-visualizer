...
| 2026-07-15 | Datenmodell/Vergleich | `buildCompareRows` und `CompareRow` liefern Parent-Anteile (`sollPctOfParent`, `istPctOfParent`) und Status `extra_in_ist` ergänzend für die deskriptive Vergleichstabelle #19 | Ermöglicht eine vollständige Gegenüberstellung von Gesamt- und Parent-Anteilen für alle Knoten in einer rein deskriptiven Tabelle | Entschieden |