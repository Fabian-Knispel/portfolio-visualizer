...
import {
  TreeView,
  collectTreeEntries,
  toggleTreeCollapseAll as toggleTreeCollapseAllInState,
  toggleTreeNodeCollapse as toggleTreeNodeCollapseInState,
} from './tree-view';

export type ViewMode = PortfolioTab;
...
export function PortfolioWorkspace({
  ...
  function toggleTreeCollapseAll(): void {
    setCollapsedTreeNodes((previous) =>
      toggleTreeCollapseAllInState(previous, expandableEntries, areAllTreeNodesCollapsed)
    );
  }

  function toggleTreeNodeCollapse(path: NodePath): void {
    setCollapsedTreeNodes((previous) => toggleTreeNodeCollapseInState(previous, path));
  }
  ...
  function handleEditorDraftChange(nextDraft: EditorDraft): void {
    if (nextDraft.label !== draft.label) {
      setRenameError(null);
    }
  ...
  function handleChildDraftChange(nextDraft: EditorDraft): void {
    if (nextDraft.childLabel !== draft.childLabel) {
      setChildPathError(null);
    }
      ...
      <div className={`workspace-body${activeViewMode === 'vergleich' ? ' workspace-body--vergleich' : ''}`}>
        {activeViewMode === 'vergleich' ? (
          <>
            <section className="panel panel--tree">
              ...
              <TreeView
                root={snapshot.sollRoot}
                activeViewMode={activeViewMode}
                selectedPath={selectedPath}
                onSelectPath={updateSelectedPath}
                collapsedNodes={collapsedTreeNodes}
                onToggleCollapse={toggleTreeNodeCollapse}
                onToggleCollapseAll={toggleTreeCollapseAll}
                sollComputedRoot={sollComputedRoot}
                istComputedRoot={istComputedRoot}
                areAllCollapsed={areAllTreeNodesCollapsed}
                readOnly
              />
            </section>
            <section className="panel panel--tree">
        ...
        {activeViewMode === 'vergleich' ? (
          <section className="panel panel--sunburst">
            ...
            <CompareTable compareRows={compareRows} />
          </section>
        ) : (
          <section className="panel panel--sunburst">
            <div className="panel__header">
          ...
          <aside className="panel panel--sidebar">
            ...
            <div className="sidebar-stack">
              <NodeDetailCard
                ...
                sollComputedRoot={sollComputedRoot}
                istComputedRoot={istComputedRoot}
                selectedIstComputedNode={selectedIstComputedNode}
                selectedSollFreeness={selectedSollFreeness}
              />
              ...
              <NodeEditor
                ...
                onSollValueInputModeChange={setSollValueInputMode}
                parentOptions={parentOptions}
                readOnly={readOnlyMode}
                isRootSollSelection={isRootSollSelection}
                draft={draft}
                onDraftChange={handleEditorDraftChange}
                renameError={renameError}
                saveSucceeded={saveSucceeded}
                editValueValidation={editValueValidation}
                onSave={handleSave}
              ...
              <AddChildForm
                ...
                childValueValidation={childValueValidation}
                childPathError={childPathError}
